...
添加好友
*/
func (server ContactServer) AddFriends(userid int64, mobile string) error {
	userTemp := model.User{}

	//查询该好友是否存在
	_, e2 := DbEngin.Where("mobile = ?", mobile).Get(&userTemp)
	if e2 != nil {
		return e2
	}
	if userTemp.Id <= 0 {
	...
	_, e := DbEngin.Where("ownerid = ?", userid).
		And("dstobj = ?", userTemp.Id).And("cate = ?", model.CONCAT_CATE_USER).Get(&temp)
